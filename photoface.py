...
class View(gtk.DrawingArea):
    ...
    def __draw_cb(self, widget, cr):
        ...
        # Eye centers
        if self._look_x is None and self._look_y is None:
            return

        # FIXME: Eyes have to be offset by allocation
        x, y, circ = self._compute_pupil(self.status.left_eye,
                                         offset_x, offset_y)
        cr.arc(x - alloc.x, y - alloc.y, circ, 0, 2 * math.pi)
        cr.set_source_rgb(0.0, 0.0, 0.0)
        cr.fill()

        x, y, circ = self._compute_pupil(self.status.right_eye,
                                         offset_x, offset_y)
        cr.arc(x - alloc.x, y - alloc.y, circ, 0, 2 * math.pi)
        cr.set_source_rgb(0.0, 0.0, 0.0)
        cr.fill()
    ...
    # Thanks to xeyes :)
    def _compute_pupil(self, eye, offset_x, offset_y):
        ...
        dx = self._look_x - EYE_X
        dy = self._look_y - EYE_Y
